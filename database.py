...
class Database:
    ...
    def init_db(self):
        with self.get_connection() as conn:
            expense_table = conn.cursor()
            
            # Create groups table
            expense_table.execute('''
    ...
    def get_group(self, group_id):
        with self.get_connection() as conn:
            expense_table = conn.cursor()
            expense_table.execute('SELECT * FROM groups WHERE id = ?', (group_id,))
            row = expense_table.fetchone()
            if row:
    ...
    def get_expenses(self, group_id):
        with self.get_connection() as conn:
            expense_table = conn.cursor()
            expense_table.execute('SELECT * FROM expenses WHERE group_id = ?', (group_id,))
            return [Expense(row[0], row[1], row[2], row[3], row[4], row[5]) for row in expense_table.fetchall()]
